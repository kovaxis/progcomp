...
struct SolState {
    ...
    int next_free = 0, next_reserved = 0;
    int time = 0;
    ...
    int where(int u, int i) {
        ...
        } else if (count_by_u[u] < Qmin[u]) {
            ans_q = next_reserved;
            cerr << "using reserved page at " << ans_q + 1 << endl;
    ...
    int apply(int ans_q, int u, int i) {
        ...
        if (was_fault) {
            if (count_by_u[u] < Qmin[u]) {
                assert(ans_q == next_reserved && !existed);
                next_reserved++;
            } else if (count_by_u[u] < Qmax[u] && next_free < Q) {
...
void reset() {
    ...
    vector<ll> mults = {0, 32, 1024};
    for (ll mult : mults) {

        // LRU (p)
        s = &sols[k++];
        s->name = "lru_p";
        s->getscore = [mult = mult](const SolState &s, int u, int i, ll p) { return p == INF ? s.time + mult * L[u] : p; };
        // LRU
        s = &sols[k++];
        s->name = "lru";
        s->getscore = [mult = mult](const SolState &s, int u, int i, ll p) { return s.time + mult * L[u]; };
        // MRU
        s = &sols[k++];
        s->name = "mru";
        s->getscore = [mult = mult](const SolState &s, int u, int i, ll p) { return -s.time + mult * L[u]; };
        ...
        s = &sols[k++];
        s->name = "lfu";
        s->getscore = [mult = mult](const SolState &s, int u, int i, ll p) { return s.u_i_to_freq[u][i] + mult * L[u]; };
        // LFRU
        s = &sols[k++];
        s->name = "lfru";
        s->getscore = [mult = mult](const SolState &s, int u, int i, ll p) { return s.u_i_to_freq[u][i] * 64 + s.time + mult * L[u]; };
        // LFRU (prefer hot)
        s = &sols[k++];
        s->name = "lfru hot";
        s->reset_freq = true;
...
int main() {
    ...
    {
        int total = 0;
        rep(i, N) total += Qbase[i];
        total -= Q;
        if (total > 0) {
            priority_queue<pair<int, int>> q;
            rep(i, N) q.push({Qbase[i], i});
            while (total > 0) {
                auto [b, i] = q.top();
                q.pop();
                if (b == Qmin[i]) continue;
                Qbase[i] -= 1;
                q.push({b - 1, i});
                total -= 1;
            }
            rep(i, N) Qmax[i] = Qbase[i];
        }
    }

    /*{
        int total = 0;
        rep(i, N) total += Qmin[i];
        int space = Q - total;

        priority_queue<pair<int, int>, vector<pair<int, int>>, greater<>> q;
        rep(i, N) q.push({Qmin[i], i});
        while (space > 0) {
            auto [b, i] = q.top();
            q.pop();
            Qmin[i] += 1;
