...
int main() {
    ...
    while (true) {
        ...
        rep(i, N) repx(j, i + 1, N) repx(k, j + 1, N) {
            if ((A[i] - A[j]) / (A[i] - A[k]) == 0) continue;
            C c = C::thru_points(A[i], A[j], A[k]);
            int cnt = 0;
            rep(l, N) {
                double d2 = (A[l] - c.o).magsq();
                if (d2 >= sq(c.r - EPS) && d2 <= sq(c.r + EPS)) cnt += 1;
            }
            // cerr << "circle at " << c.o << " with radius " << c.r << " touches " << cnt << " points" << endl;
            ans = max(ans, cnt);
        }
