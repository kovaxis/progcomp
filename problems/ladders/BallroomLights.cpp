// https://www.beecrowd.com.br/judge/en/problems/view/1137
...
void merge(Ring &ring) {
    sort(ring.evs.begin(), ring.evs.end());
    vector<pair<double, int>> out;
    int acc = ring.base;
    for (int i = 0; i < ring.evs.size();) {
        double x = ring.evs[i].first;
        int prev = acc;
        for (; i < ring.evs.size() && ring.evs[i].first == x; i++) {
            acc += ring.evs[i].second;
        }
        if (prev == 0 && acc != 0) out.push_back({x, 1});
        if (prev != 0 && acc == 0) out.push_back({x, -1});
    }
    ring.evs = out;
    ring.base = ring.base != 0;
}
...
int main() {
    ...
    while (true) {
        ...
        rep(j, M) cin >> cols[j].o >> cols[j].r;

        vector<L> B = {L({0, 0}, {W, 0}), L({W, 0}, {0, H}), L({W, H}, {-W, 0}), L({0, H}, {0, -H})};

        Ring lighted;
        rep(i, N) {
            Ring shadow;
            rep(j, M) {
                pair<P, P> tp = cols[j].tangents(lights[i]);
                P t[2];
                t[0] = tp.first, t[1] = tp.second;
                rep(k, 2) {
                    L ray = L(lights[i], t[k] - lights[i]);
                    double x;
                    if (ray.ray_seg_collide(B[0])) x = ray.intersection(B[0]).x;
                    else if (ray.ray_seg_collide(B[1])) x = W + ray.intersection(B[1]).y;
                    else if (ray.ray_seg_collide(B[2])) x = W + H + W - ray.intersection(B[2]).x;
                    else x = W + H + W + H - ray.intersection(B[3]).y;
                    t[k].x = x;
                }
                shadow.evs.push_back({t[0].x, 1});
                shadow.evs.push_back({t[1].x, -1});
                if (t[0].x > t[1].x) shadow.base += 1;
            }
            cerr << "pre-merge shadow for light " << i + 1 << " is: (" << shadow.base << ")";
            for (auto [op, cl] : shadow.evs) cerr << " [" << op << ", " << cl << "]";
            cerr << endl;
            merge(shadow);
            cerr << "shadow for light " << i + 1 << " is: (" << shadow.base << ")";
            for (auto [op, cl] : shadow.evs) cerr << " [" << op << ", " << cl << "]";
            cerr << endl;
            for (auto &p : shadow.evs) p.second *= -1;
            shadow.base = !shadow.base;
            lighted.base += shadow.base;
            lighted.evs.insert(lighted.evs.end(), shadow.evs.begin(), shadow.evs.end());
        }
        cerr << "pre-merge lighted is: (" << lighted.base << ")";
        for (auto [op, cl] : lighted.evs) cerr << " [" << op << ", " << cl << "]";
        cerr << endl;
        merge(lighted);
        cerr << "total lighted is: (" << lighted.base << ")";
        for (auto [op, cl] : lighted.evs) cerr << " [" << op << ", " << cl << "]";
        cerr << endl;

        double ans = 0;
        int acc = lighted.base;
        if (lighted.evs.empty() && acc) ans += 2 * (W + H);
        rep(i, lighted.evs.size()) {
            acc += lighted.evs[i].second;
            if (acc == 0) continue;
            int j = (i + 1) % lighted.evs.size();
            double len = lighted.evs[j].first - lighted.evs[i].first;
            if (len < 0) len += 2 * (W + H);
            ans += len;
        }
