// https://open.kattis.com/problems/tomosynthesis
...
struct P {
    ...
    static P from_angle(T a) { return {cos(a), sin(a)}; }
};
...
int main() {
    ...
    cin >> N;
    A.resize(N);
    rep(i, N) cin >> A[i].o >> A[i].r;

    vector<pair<P, int>> B;
    int base = 0;
    rep(i, N) repx(j, i + 1, N) {
        P s = A[i].tangent(A[j], -1, -1).d;
        P e = A[i].tangent(A[j], -1, 1).d;
        B.push_back({s, 1}), B.push_back({e, -1});
        B.push_back({-s, 1}), B.push_back({-e, -1});
        cerr << "between cylinders " << i + 1 << " and " << j + 1 << endl;
        cerr << "shadow at [" << s.angle() * 180 / M_PI << ", " << e.angle() * 180 / M_PI << "]" << endl;
        cerr << "shadow at [" << (-s).angle() * 180 / M_PI << ", " << (-e).angle() * 180 / M_PI << "]" << endl;
        base += (e.angcmp(s) < 0);
        base += ((-e).angcmp(-s) < 0);
    }
    ...
    sort(B.begin(), B.end(), [](auto a, auto b) {
        return a.first.angcmp(b.first) < 0;
    });
    ...
    for (int j = 0; j < B.size();) {
        int i = j, old = acc;
        while (j < B.size() && abs(B[j].first.angcmp(B[i].first)) <= EPS) {
            cerr << acc << " "
                 << "- +"[B[j].second + 1] << "1 at " << atan2(B[j].first.y, B[j].first.x) * 180 / M_PI << endl;
            acc += B[j++].second;
        }
        if (acc && !old) M.push_back({B[i].first, 1});
        if (!acc && old) M.push_back({B[i].first, -1});
    }
    ...
    rep(i, M.size()) {
        int j = (i + 1) % M.size();
        if (M[i].second == -1) {
            P a = M[i].first.unit(), b = M[j].first.unit();
            double da = atan2(a / b, a * b);
            while (da < 0) da += 2 * M_PI;
            while (da >= 2 * M_PI) da -= 2 * M_PI;
            cerr << "da = " << da * 180 / M_PI << " between " << a.angle() * 180 / M_PI << " and " << b.angle() * 180 / M_PI << endl;
            ans = max(ans, da);
        }
    }

    if (ans <= EPS) cout << "0" << endl;
