// https://www.beecrowd.com.br/judge/en/problems/view/1297
...
double eval_y(int i, double x) {
    double pq[2];
    rep(j, 2) {
        pq[j] = 0;
        double xx = 1;
        rep(k, K + 1) pq[j] += xx * C[i][j][k], xx *= x;
    }
    return pq[0] / pq[1];
}

double eval_dy(double x, double d) {
    double top = max(eval_y(0, x), -d);
    double bot = max(eval_y(1, x), -d);
    return top - bot;
}

double eval_a(double d) {
    const int PREC = 1e5;
    double a = 0;
    rep(i, PREC) {
        double x = W / PREC * (i + 0.5);
        a += eval_dy(x, d);
    }
    a *= W / PREC;
    // cerr << "depth " << fixed << setprecision(16) << d << " evaluates to area " << a << fixed << setprecision(16) << endl;
    return a;
}
...
int main() {
    ...
    while (true) {
        Case t;
        if (!(cin >> t.W >> t.D >> t.A >> t.K)) break;
        rep(i, 2) rep(j, 2) t.C[i][j].resize(t.K + 1);
        rep(i, 2) rep(j, 2) rep(k, t.K + 1) cin >> t.C[i][j][k];
        TC.push_back(t);
    }

    double middle = now();
    double per_test = (avail - middle) / TC.size();

    for (Case &t : TC) {
        W = t.W, D = t.D, A = t.A, K = t.K;
        rep(i, 2) rep(j, 2) swap(C[i][j], t.C[i][j]);

        double l = 0, r = D;
        double deadline = now() + per_test;
        while (now() < deadline) {
            double m = (l + r) / 2;
            double a = eval_a(m);
            if (a < A) l = m;
