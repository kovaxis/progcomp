...
int main() {
    ...
    sort(D.begin(), D.end(), [](P a, P b) {
        return a.x + a.y < b.x + b.y;
    });
    sort(D.begin(), D.begin() + K, [](P a, P b) {
        return a.angcmp(b) < 0;
    });
    ...
    P u((ll)2e7, 0);
    rep(k, 4) {
        cerr << "building with D.size = " << D.size() << endl;
        rep(i, K) {
            V.push_back(u);
            P d = D[i];
            rep(kk, k) d = d.rot();
    ...
    // rep(i, V.size()) if (i % 1000 == 0) cout << V[(i + 1000) % V.size()] - V[i] << "\n";
