// https://www.beecrowd.com.br/judge/en/problems/view/2015
...
int main() {
    ...
    cerr << "total volume " << total << endl;

    int opp = 0;
    ll best = total;
    rep(i, N) {
        while (true) {
            ll v = cur + (A[opp] - A[i]) / (A[nxt(opp)] - A[i]);
            if (min(v, total - v) < min(cur, total - cur)) break;
            cur = v;
            opp = nxt(opp);
        }
        cerr << i + 1 << " at " << A[i] << " matches with " << opp + 1 << " at " << A[opp] << ", with volume " << cur << endl;
        best = min(best, min(cur, total - cur));
        cur -= (A[nxt(i)] - A[i]) / (A[opp] - A[i]);
    }
