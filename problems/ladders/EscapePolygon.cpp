// https://www.beecrowd.com.br/judge/en/problems/view/2907
...
int main() {
    ...
    A.resize(N);
    rep(i, N) A[i] = (poly[nxt(i)] - poly[i]).rot();

    int j = 0;
    ll escape = 0;
    rep(i, N) {
        while (nxt(j) != i && A[i] / A[nxt(j)] >= 0) j = nxt(j);
        ll left = (j - i + N) % N;
        escape += left * (left - 1) / 2;
    }

    ll total = (ll)N * (N - 1) * (N - 2) / 6;

    cout << total - escape << endl;
}
