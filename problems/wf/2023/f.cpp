...
pair<int, int> findshifts(string &b, string &f) {
    int n = b.size();
    assert(n == (int)f.size());
    b.push_back('$');
    ...
    int j;
    repx(i, 1, 3 * n + 1) {
        for (j = p[i - 1]; j > 0 && b[j] != b[i];) j = p[j - 1];
        p[i] = j + (b[j] == b[i]);
    }
    int m0 = -1, m1 = -1;
    repx(i, 2 * n, 3 * n + 1) {
...
void checkloop(const Board &b) {
    ...
    rep(i0, N) rep(j0, M) if (tmp[0].a[i0][j0] != '.') {
        ...
        do {
            bstr.push_back(b[cur]);
            fstr.push_back(fin[cur]);
            tmp[0].a[cur.first][cur.second] = '.';
            cur = arr[cur];
        } while (cur != Pos{i0, j0});
        auto [m0, m1] = findshifts(bstr, fstr);
        // cerr << "  loop at " << i0 << " " << j0 << " with len " << fstr.size() << ", b = " << bstr << ", f = " << fstr << ", matches = {" << m0 << ", " << m1 << "}" << endl;
        if (m0 == -1) return;
        if (m1 == -1) m1 = m0 + fstr.size();
        int m = m1 - m0;
        // t === m0 (mod m)
        factorize(m, f);
