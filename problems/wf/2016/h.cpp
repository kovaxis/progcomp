...
static void svgprint(ostream& s) {
    ...
    s << "<svg width=\""<<ceil(svgmx.x-svgmn.x)<<"\" height=\""<<ceil(svgmx.y-svgmn.y)<<"\" xmlns=\"http://www.w3.org/2000/svg\">\n";
    for(auto [x, color] : svglines) {
        auto [a, b] = x;
        a = a - svgmn;
        b = b - svgmn;
        s << "  <line x1=\"" << a.x << "\" y1=\""<<a.y<<"\" x2=\""<<b.x<<"\" y2=\""<<b.y<<"\" style=\"stroke:"<<color<<"\" />\n";
    }
    s << "</svg>" << endl;
}
...
void svgline(P a, P b, string color) {
    a.y *= -1, b.y *= -1;
    a = a + svgdelta, b = b + svgdelta;
    svglines.push_back({{a, b}, color});
}
...
void trypolys() {
    int poplen = svglines.size();
    double oldans = answer;
    if (!check() || answer == oldans) {
        svgline({-10, -10}, {10, 10}, "red");
        svgline({-10, 10}, {10, -10}, "red");
        svglines.resize(poplen);
    } else {
        svgdelta = svgdelta + P{0, 250};
    }
}
...
int main() {
    ...
    cerr << fixed << setprecision(12) << answer << endl;
