// https://open.kattis.com/problems/rocketpoweredhovercraft
...
struct C {
    ...
    pair<P, P> line_inter(L l) const {
        P c = l.closest_to(o);
        T c2 = (c - o).magsq();
        P e = sqrt(max(r * r - c2, 0.)) * l.d.unit();
        return {c - e, c + e};
    }
    ...
    // the tangents are sorted by angle with respect to the point
...
// get the time taken to reach the destination provided that we
// stop turning at angle `w`
// turning starts at the beggining, thrusting stop at the end,
// and the moment to start thrusting is derived from the other parameters
double f(double w) {
    // 2*pi*r / V = 2*pi / W
    double r = V / W;
    L l = L(D, P(cos(w), sin(w)));
    if (l.side(P()) > 0) return INFINITY;
    C c = C::thru_point_line_r({0, 0}, l, r).second;
    P u = c.o - l.d.rot() * c.r;
    double time = w / W + (D - u).mag() / V;
    cerr << "rotating for " << w / W << " seconds until " << w * 180 / M_PI << " degrees at point " << u << ", then going forward for " << (D - u).mag() / V << "s, totalling " << time << "s" << endl;
    return time;
}
...
double findmin(double l, double r) {
    ...
    double m = l + (r - l) * INVG;
    double lv = f(l), rv = f(r), mv = f(m);
    rep(i, 100) {
        cerr << "l = f(" << l << ") = " << lv << ", m = f(" << m << ") = " << mv << ", r = f(" << r << ") = " << rv << endl;
        double x = l + (m - l) * INVG;
        double xv = f(x);
...
double solve() {
    double R = V / W, r;
    cerr << "turn radius is " << R << endl;
    C s({0, R}, R);
    if ((D - s.o).magsq() <= R * R) {
        C c = C::thru_points_r({0, 0}, D, R).second;
        P d = (D - c.o).rot();
        r = atan2(d.y, d.x);
    } else {
        P t = s.tangents(D).second;
        P d = D - t;
        r = atan2(d.y, d.x);
    }
    r += 2 * M_PI;
