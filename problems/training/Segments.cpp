...
bool bellman_ford(int N, int s, vector<Edge> &E, vector<ll> &D) {
    D.assign(N, (ll)1e18), D[s] = 0;
    rep(i, N - 1) {
        bool f = true;
        rep(ei, E.size()) {
            auto &e = E[ei];
            ll n = D[e.u] + e.w;
            if (D[e.u] < (ll)1e18 && n < D[e.v]) D[e.v] = n, f = false;
        }
        if (f) return false;
    }
    return true;
...
bool can_solve(int R) {
    ...
    rep(j, M) E.push_back({M, j, 0});
    vector<ll> D;
    if (bellman_ford(M + 1, M, E, D)) {
        cerr << "R = " << R << " cannot be solved" << endl;
        return false;
    }
    cerr << "solution with R = " << R << " found:";
    rep(i, M - 1) cerr << " " << D[i + 1] - D[i];
    cerr << endl;
    return true;
}
...
int main() {
    ...
    rep(i, N) A[i] = {coords[A[i].first], coords[A[i].second]};

    cerr << "segments:";
    rep(i, N) cerr << " [" << A[i].first << " " << A[i].second << "]";
    cerr << endl;
