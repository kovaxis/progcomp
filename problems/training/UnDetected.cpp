...
void join(int from, int to) {
    cerr << "joining " << from << " with " << to << endl;
    from = color[from];
    to = color[to];
    rep(i, N + 2) if (color[i] == to) color[i] = from;
    cerr << "colors:";
    rep(i, N + 2) cerr << " " << color[i];
    cerr << endl;
}
...
int main() {
    ...
    rep(i, N) {
        // add circle `i`
        if (a[i].o.x - a[i].r <= 0) join(N, i);
        if (a[i].o.x + a[i].r >= 200) join(N + 1, i);
        rep(j, i) if (a[i].collide(a[j])) join(i, j);

        // check if edges are joined
        if (ans == -1 && color[N] == color[N + 1]) {
            ans = i;
        }
    }
