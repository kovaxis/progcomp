...
int main() {
    ...
    cin >> N >> Q;
    X.resize(N), S.resize(N), E.resize(N);
    D.resize(Q);
    rep(i, N) cin >> S[i] >> E[i] >> X[i];
    rep(q, Q) cin >> D[q];

    rep(i, N) {
        // s <= d + x < t
        // l <= d < r
        int l = S[i] - X[i], r = E[i] - X[i];
        evs.push_back({l, X[i]});
        evs.push_back({r, -X[i]});
        // cerr << "roadwork at " << X[i] << " in timespan [" << S[i] << ", " << E[i] << ") affects people starting within timestamp [" << l << ", " << r << ")" << endl;
    }
    ...
    set<int> active;
    for (int d : D) {
        for (; j < evs.size() && evs[j].first <= d; j++) {
            if (evs[j].second > 0) {
                active.insert(evs[j].second);
            } else {
                active.erase(-evs[j].second);
            }
        }

        // cerr << "active roadworks for person starting at " << d << ":";
        // for (int x : active) cerr << " " << x;
        // cerr << endl;
