...
void add_if(ll D2, int i, int j, ll d) {
    if (4 * d * d < D2) G[i].push_back(j), G[j].push_back(i);
}

bool can_cross(ll D2) {
    G.assign(N + 2, {});
    rep(i, N) {
        repx(j, i + 1, N) if ((A[i] - A[j]).magsq() < D2) {
            G[i].push_back(j);
            G[j].push_back(i);
        }
        add_if(D2, i, N, A[i].x);
        add_if(D2, i, N + 1, W - A[i].x);
        add_if(D2, i, N + 1, A[i].y);
        add_if(D2, i, N, H - A[i].y);
    }

    seen.assign(N + 2, false);
    mark(N);

    // cerr << "can cross with sqrt(" << D2 << ") / 2 distance: " << !seen[N + 1] << endl;
...
int main() {
    ...
    while (true) {
        ...
        cout << fixed << setprecision(3) << sqrt((double)l) / 2 << "\n";
    }
}
