...
int main() {
    ...
    while (true) {
        ...
        rep(i, N) repx(j, i + 1, N) repx(k, j + 1, N) {
            if (abs((a[i] - a[j]) / (a[i] - a[k])) <= EPS) continue;
            P o = C::thru_points(a[i], a[j], a[k]).o;
            // cerr << "trying center at " << o << endl;
            bool in = true;
            rep(n, N) if (!is_in(a[n] - o)) in = false;
            if (in) ok = true;
        }
        ...
        if (ok) cout << "The polygon can be packed in the circle.\n";
