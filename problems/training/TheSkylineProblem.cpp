...
int main() {
    ...
    while (true) {
        B b;
        if (!(cin >> b.l >> b.h >> b.r)) break;
        a.push_back(b);
    }

    vector<vector<Ev>> evs(10009);
    for (B b : a) evs[b.l].push_back({1, b.h});
    for (B b : a) evs[b.r].push_back({-1, b.h});
    ...
    rep(x, evs.size()) {
        for (Ev ev : evs[x]) {
            if (ev.ty == 1) {
                ...
                active[ev.h] -= 1;
                if (active[ev.h] == 0) active.erase(ev.h);
            }
        }
        int h = 0;
        if (!active.empty()) h = (--active.end())->first;
        if (h != prev_h) {
