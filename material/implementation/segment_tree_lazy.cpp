...
struct Stl3 {
    ...
    void reset(int N) { node.assign(4 * N, {qneutral(), uneutral()}); } // node neutral

    void push(int v, int vl, int vm, int vr) {
        update(node[2 * v], node[v].second, vl, vm);     // node update
        update(node[2 * v + 1], node[v].second, vm, vr); // node update
        node[v].second = uneutral();                     // update neutral
    }

    // query for range [l, r)
    T query(int l, int r, int v = 1, int vl = 0, int vr = -1) {
        if (vr == -1) vr = node.size() / 4;
        if (l <= vl && r >= vr) return node[v].first; // query op
    ...
    void update(int l, int r, T val, int v = 1, int vl = 0, int vr = -1) {
        ...
        if (l <= vl && r >= vr) update(node[v], val, vl, vr); // node update
        else {
            int vm = (vl + vr) / 2;
            push(v, vl, vm, vr);
            update(l, r, val, 2 * v, vl, vm);
            update(l, r, val, 2 * v + 1, vm, vr);
            node[v].first = merge(node[2 * v].first, node[2 * v + 1].first); // node merge
        }
    }
};
